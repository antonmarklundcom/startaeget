# Phase S4 — Starta företag cluster. SONNET session. Lane 2, runs in parallel with the other S phases.

Read ONLY: this file, `plan.md` §1, §2.2, §4, §6 (common rules) and §6.1, the phase table and §9, `docs/log/O2.md`, `docs/log/O3.md`, and the two O2 exemplar articles as the quality reference. Do not read the rest.
...
- `content/articles/starta-foretag/**` (except `starta-aktiebolag.mdx`, written in O2 — do not rewrite it)
- `content/image-briefs/s4.json`, `docs/log/S4.md`
...
- Slugs: legacy pages use the exact path from `content/legacy-urls.json`; if the file is still `"complete": false` and a legacy slug you need is missing, use the slug named in plan §6.1 and log it under Known issues.
...
- Cornerstone first: `starta-eget-foretag` (legacy). `enskild-firma-eller-aktiebolag` must agree with tool 1's logic — read `content/tools.ts` and the tool copy, do not contradict it.
- The byrå lead form block goes on every article in this hub (template flag in frontmatter, see O2 log).
- Re-runnable; minor issues → `docs/log/S4.md`; stop only per §4.4.

Exit: every article in §6.1 exists and renders; verify green (frontmatter validation passes, sitemap lists them all); one screenshot pass (≤ 5 pages × 2 widths); PR merged; `docs/log/S4.md`.
